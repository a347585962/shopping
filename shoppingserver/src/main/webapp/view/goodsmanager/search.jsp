			...
			<table style="width: 100%;" class="table table-hover article-table">
					<c:choose>
						<c:when test="${empty goodsinfos }">
							<tr>
								<td colspan="3" style="color: red ;text-align:center" >亲！抱歉，没有相关商品，请核对查询信息。</td>
							</tr>
						</c:when>
						<c:when test="${not empty goodsinfos}">
							...
							<c:forEach items="${goodsinfos }" var="goodsinfo" varStatus="status">
								<tr class='discuss'>
									<td class="content_td1 ">
										<div class='newsTitle1'>
											<a title='${goodsinfo.goodsinfo.goodsId}' href="<%=basePath%>goodsmanager/updateview/${goodsinfo.shopgoodsId}"> <span>${goodsinfo.shopgoodsId}</span>
											</a>
										</div>
									</td>
								...
								<button  url="<%=basePath %>goodsmanager/delete/${goodsinfo.shopgoodsId }"  style='width:80px;'
